...
usePySide = False
if usePySide :
	from PySide import QtCore, QtGui, QtWebEngineWidgets, QtNetwork
else :
	from PyQt5 import QtCore, QtGui, QtWebEngine, QtNetwork, QtWebEngineWidgets
...
class _LoggedPage(QtWebEngineWidgets.QWebEnginePage):
	@trace
...
class QGoogleMap(QtWebEngineWidgets.QWebEngineView) :
	...
	def __init__(self, parent, debug=True) :
		...
		if debug :
			#QtWebEngineWidgets.QWebEngineSettings.globalSettings().setAttribute(
			#	QtWebEngineWidgets.QWebEngineSettings.DeveloperExtrasEnabled, True)
